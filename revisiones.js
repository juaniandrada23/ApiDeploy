...
routes.get('/:idpacientes1', (req, res)=>{
    req.getConnection((err, conn)=>{
        ...
        conn.query(`SELECT numeroRevision, descripcion, dosis, medicacion, fecha, paciente FROM revisiones INNER JOIN pacientes ON revisiones.paciente = pacientes.idpacientes1 WHERE idpacientes1 = ?`,idpacientes1,(err, rows)=>{
            if(err) return res.send(err)
            res.json(rows)
        })
    })
